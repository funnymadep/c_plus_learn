...
int main() {
    ...
    std::string gst_pipeline = "rtspsrc location=" + rtsp_url + " latency=0 ! rtph264depay ! h264parse ! avdec_h264 ! videoconvert ! appsink";

    cv::VideoCapture cap(gst_pipeline, cv::CAP_GSTREAMER);
    if (!cap.isOpened()) {
        std::cerr << "Failed to open GStreamer pipeline. Exiting" << std::endl;
